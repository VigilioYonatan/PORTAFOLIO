...
const CATEGORY_MAP: Record<string, { icon: any; color: string; accent: string }> = {
	FRONTEND: { icon: LayoutIcon, color: "text-primary", accent: "rgba(6,182,212,0.6)" },
	BACKEND: { icon: Layers, color: "text-amber-500", accent: "rgba(245,158,11,0.6)" },
	DATABASE: { icon: DatabaseIcon, color: "text-emerald-500", accent: "rgba(16,185,129,0.6)" },
	DEVOPS: { icon: InfinityIcon, color: "text-blue-500", accent: "rgba(59,130,246,0.6)" },
	LANGUAGE: { icon: Code2, color: "text-zinc-400", accent: "rgba(161,161,170,0.6)" },
	MOBILE: { icon: SmartphoneIcon, color: "text-rose-500", accent: "rgba(244,63,94,0.6)" },
...
export default function SkillBentoGrid() {
	...
	const technologyQuery = technologyIndexApi(null, { limit: 12 });
	...
	if (technologyQuery.isLoading) {
		component = (
			<div class="flex flex-col justify-center items-center h-64 gap-4">
                ...
                <span class="text-[10px] font-mono text-primary/60 tracking-[0.4em] animate-pulse">SYSTEM_SCANNING...</span>
			</div>
		);
	}
	...
	if (technologyQuery.isError) {
		component = (
			<div class="text-center py-12 text-destructive border border-destructive/20 bg-destructive/5 rounded-sm p-8 font-mono">
				<p class="text-[10px] tracking-widest uppercase font-black">[ CRITICAL_ERROR: FAIL_TO_RETRIEVE_CAPABILITIES ]</p>
                <p class="text-[8px] opacity-60 mt-2">{technologyQuery.error?.message}</p>
			</div>
		);
	}
						...
						return (
							<SkillCard
								...
								skill={{
									...
									tags: tech.category === "AI" ? ["LLM", "RAG"] : ["Architecture", "Scalability"],
									color: catInfo.color,
			...
			<div class="max-w-7xl mx-auto relative z-10">
				<div class="flex flex-col items-center mb-20 text-center">
                    <div class="flex items-center gap-4 mb-4">
					    ...
					    <h3 class="text-[10px] font-black font-mono text-primary tracking-[0.8em] uppercase text-glow">
		...
		>
			...
			<div class="relative z-10">
				...
				>
					...
					<Icon size={24} class="relative z-10 transition-transform group-hover:scale-110" />
				</div>
				...
				<div class="flex items-center gap-3 mb-6">
					...
					<span class="text-[9px] font-black font-mono text-primary/60 tracking-widest uppercase">
						{skill.level}
					</span>
				</div>
			</div>
			...
			<div class="relative z-10 mt-auto">
				<div class="flex flex-wrap gap-2">
					{skill.tags.map((tag) => (
						<span
							key={tag}
							class="text-[9px] font-black font-mono px-2 py-0.5 bg-zinc-900/80 border border-white/10 text-zinc-500 group-hover:text-primary/80 group-hover:border-primary/30 transition-all rounded-[2px] tracking-widest uppercase"
						>
							#{tag}
						</span>
					))}
				...
				<div class="mt-6 flex items-center justify-between opacity-40 group-hover:opacity-100 transition-all translate-y-2 group-hover:translate-y-0 duration-500">
					<div class="flex items-center gap-2">
                        ...
                        <span class="text-[8px] text-primary/80 font-black font-mono tracking-[0.2em]">
						    STATUS: OPTIMIZED
					    </span>
                    </div>
					<ExternalLink size={12} class="text-primary/40 hover:text-primary transition-colors cursor-pointer" />
				</div>
			</div>
