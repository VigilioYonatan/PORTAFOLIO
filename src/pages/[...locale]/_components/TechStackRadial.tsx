...
const TECH_GROUPS = [
    { name: "FRONTEND", items: ["React", "Astro", "Preact", "Tailwind"], color: "#06b6d4" },
    { name: "BACKEND", items: ["Node", "NestJS", "Go", "Rust"], color: "#3b82f6" },
    { name: "SYSTEMS", items: ["C++", "Linux", "Docker", "K8s"], color: "#ef4444" },
    { name: "DATABASE", items: ["Postgres", "Redis", "Mongo", "Elastic"], color: "#10b981" }
];

export default function TechStackRadial({ class: className }: TechStackRadialProps) {
    ...
    useEffect(() => {
        ...
        const draw = () => {
            ...
            const gradient = ctx.createRadialGradient(displayCenterX, displayCenterY, 0, displayCenterX, displayCenterY, displayRadius * 2);
            ...
            TECH_GROUPS.forEach((group, gIdx) => {
                ...
                const groupFreqIndex = Math.floor((gIdx / TECH_GROUPS.length) * freq.length);
                ...
                group.items.forEach((item, iIdx) => {
                    ...
                    const nextAngle = ((iIdx + 1) / group.items.length) * Math.PI * 2 + rotation;
    ...
    return (
        <div class={cn("relative flex items-center justify-center p-8 min-h-[500px] overflow-hidden", className)}>
            ...
            <div class="relative z-10 flex items-center justify-center pointer-events-none">
                <div class="text-center p-6 border border-primary/20 bg-black/40 backdrop-blur-md rounded-full w-32 h-32 flex flex-col items-center justify-center group">
                    ...
                    <div class="text-[8px] font-black font-mono text-primary/60 uppercase tracking-[0.4em] mb-1">core_sys</div>
                    <div class="text-[12px] font-black tracking-[0.2em] text-white selection:bg-primary selection:text-black">KERNEL_v4</div>
            ...
            <div class="absolute bottom-4 left-4 font-mono text-[8px] text-zinc-500 tracking-widest uppercase flex flex-col gap-1 pointer-events-none">
                ...
                <span class="text-primary/40">FREQ_SYNC: {frequencyData.value.length} BINS</span>
            </div>
        </div>
    );
}
