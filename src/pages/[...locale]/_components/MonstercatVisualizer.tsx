...
export default function MonstercatVisualizer({
	...
	useEffect(() => {
        ...
        const updateColor = () => {
             const style = getComputedStyle(document.documentElement).getPropertyValue("--primary").trim();
             if (style) {
                 cachedPrimaryColor = style.startsWith("#") || style.startsWith("rgb") ? style : `rgb(${style})`;
             }
        };
        // Initial fetch
		...
		const render = () => {
			...
			for (let i = 0; i < barCount; i++) {
				...
				const boostedVal = Math.pow(normalizedVal, 1.5) * (1 - distFromCenter * 0.3); 
