...
export default function CommentList() {
	const updateMutation = socialCommentUpdateApi();
	const table = useTable({ columns: [], pagination: { limit: 10 } });
	const query = socialCommentIndexApi();
	...
	return (
		<div class="space-y-4">
			<div class="flex items-center gap-2 mb-4">
				...
				<h3 class="text-xs font-black font-mono tracking-[0.3em] uppercase">
					MODERATION_QUEUE
				</h3>
			</div>
			...
			<div class="grid gap-4">
				{query.isLoading ? (
					...
					query.data?.results.map((comment: any) => (
						<div
							key={comment.id}
							class="bg-zinc-900/30 border border-white/5 p-4 rounded-xl flex items-start justify-between gap-4 group hover:bg-white/[0.02] transition-all"
						>
								...
								<button
									onClick={() => {
										updateMutation.mutate(
											{
												id: comment.id,
												body: { is_visible: !comment.is_visible },
											},
											{
												onSuccess() {
													sweetModal({
														icon: "success",
														title: comment.is_visible
															? "Node Hidden"
															: "Node Approved",
													});
													query.refetch(false);
