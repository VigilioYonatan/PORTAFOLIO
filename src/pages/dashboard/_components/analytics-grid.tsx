...
export function AnalyticsGrid({ metrics, lang = "es" }: AnalyticsGridProps) {
	...
	const metricsData = [
		{
			title: t("dashboard.analytics.views"),
			value: formatNumber(metrics?.totalViews ?? 0),
			sub: `+20.1% ${t("dashboard.analytics.gain")}`,
			icon: Eye,
		},
		{
			title: t("dashboard.analytics.recruiters"),
			value: `+${formatNumber(metrics?.totalUsers ?? 0)}`,
			sub: `+180.1% ${t("dashboard.analytics.active")}`,
			icon: Users,
		},
		{
			title: t("dashboard.analytics.threads"),
			value: formatNumber(metrics?.totalChats ?? 0),
			sub: `+19.2% ${t("dashboard.analytics.burst")}`,
			icon: Activity,
		},
		{
			title: t("dashboard.analytics.integrity"),
			value: "99.9%",
			sub: t("dashboard.analytics.stable"),
			...
			<div class="grid gap-6 md:grid-cols-2 h-[420px]">
						...
						<div class="flex items-center justify-between">
							<div class="flex items-center gap-3">
								...
								<CardTitle className="text-[10px] font-black tracking-[0.3em] uppercase text-white">
									{t("dashboard.insights.recruiter_radar")}
								</CardTitle>
							</div>
							<span class="text-[8px] text-primary/40 font-mono">
								{t("dashboard.insights.live_flow")}
							</span>
						</div>
					</CardHeader>
				...
				<Card className="bg-zinc-950/40 border-white/5 relative group hover:border-primary/20 transition-all">
					...
					<CardHeader className="relative z-10">
						<div class="flex items-center justify-between">
							<CardTitle className="text-[10px] font-black tracking-[0.3em] uppercase text-white">
								{t("dashboard.insights.ai_signals")}
							</CardTitle>
						...
						<div class="flex flex-col gap-6">
							<div class="space-y-3">
								<div class="flex flex-wrap gap-2">
									{["Microservicios", "WebGL", "Patrón_RAG", "K8s"].map(
										(tag) => (
											<span
												key={tag}
								...
								<div class="p-4 bg-black/40 border border-white/5 rounded-sm relative overflow-hidden">
									...
									<p class="text-[11px] text-muted-foreground leading-relaxed font-mono tracking-tight uppercase">
										&gt; {t("dashboard.insights.high_interest")}{" "}
										<span class="text-white font-bold">
											{t("dashboard.insights.reactive_arch")}
										</span>{" "}
										y{" "}
							...
							<div class="space-y-3">
								<span class="text-[9px] font-black text-primary/60 tracking-[0.3em] uppercase flex items-center gap-2">
									<Activity size={10} />
									{t("dashboard.insights.rec_protocols")}:
								</span>
								<ul class="space-y-2">
									{[
										t("dashboard.insights.actions.1"),
										t("dashboard.insights.actions.2"),
										t("dashboard.insights.actions.3"),
									].map((action) => (
										<li
											key={action}
