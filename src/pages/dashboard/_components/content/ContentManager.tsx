...
import CategoryManager from "@modules/blog-category/components/CategoryManager";
import PostBentoGrid from "@modules/blog-post/components/PostBentoGrid";
import ProjectTable from "@modules/project/components/ProjectTable";
import { useSignal } from "@preact/signals";
import { useEffect } from "preact/hooks";
import { useLocation } from "wouter-preact";
import { LayoutGrid, List, DatabaseIcon, FileCodeIcon, ArchiveIcon } from "lucide-preact";
		...
		<div class="flex flex-col gap-8 animate-in fade-in duration-500 relative">
			...
			<div class="flex flex-col md:flex-row md:items-center justify-between gap-8 border-b border-white/5 pb-8">
				<div class="flex items-center gap-4">
                    ...
                    <div>
					    ...
					    <div class="flex items-center gap-3 mt-1">
                            <span class="text-[9px] text-primary font-bold font-mono uppercase tracking-[0.4em]">Resource_Manager</span>
                            <span class="w-1 h-1 bg-white/20 rounded-full" />
                            <span class="text-[9px] text-muted-foreground font-mono uppercase tracking-[0.4em]">Content_Sync_Active</span>
                        </div>
                    </div>
				</div>
				...
				<div class="flex items-center gap-2 bg-zinc-950/60 p-1.5 border border-white/10 rounded-sm backdrop-blur-md">
					<button
						onClick={() => (view.value = "nodes")}
						class={cn(
							"flex items-center gap-3 px-6 py-2.5 transition-all font-mono text-[10px] font-black uppercase tracking-widest rounded-sm relative group",
							view.value === "nodes"
								? "bg-primary text-black shadow-glow"
								: "text-muted-foreground hover:text-white hover:bg-white/5",
						)}
					>
						<DatabaseIcon size={12} class={cn(view.value === "nodes" ? "text-black" : "text-primary/40")} /> 
                        PROJECT_NODES
					</button>
					<button
						onClick={() => (view.value = "articles")}
						class={cn(
							"flex items-center gap-3 px-6 py-2.5 transition-all font-mono text-[10px] font-black uppercase tracking-widest rounded-sm relative group",
							view.value === "articles"
								? "bg-primary text-black shadow-glow"
					...
					>
						<FileCodeIcon size={12} class={cn(view.value === "articles" ? "text-black" : "text-primary/40")} /> 
                        BLOG_ARTICLES
					</button>
				</div>
			</div>
					...
					<div class="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
						<div class="lg:col-span-3">
							<div class="sticky top-24 bg-zinc-950/40 border border-white/5 p-4 rounded-sm backdrop-blur-md">
                                <div class="mb-4 px-2 flex justify-between items-center text-[9px] font-black font-mono tracking-[0.2em] text-zinc-500 uppercase">
