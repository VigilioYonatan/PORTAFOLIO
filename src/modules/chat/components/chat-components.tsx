...
export function ConversationList() {
	...
	if (isError) {
		component = (
			<div class="flex-1 flex items-center justify-center">
				<div class="text-destructive text-sm">Error cargando sesiones</div>
			</div>
		);
	}
		...
		<div class="border-r border-border h-full flex flex-col bg-card/50">
			<div class="p-4 border-b border-border">
				<h3 class="font-bold flex items-center gap-2">
					<MessageSquareIcon size={18} /> Sesiones
				</h3>
			</div>
...
export function ChatViewer() {
	...
	useEffect(() => {
		if (messagesData?.messages) {
			messages.value = messagesData.messages as unknown as ChatMessageDisplay[];
		}
	}, [messagesData]);
		...
		return (
			<div class="flex flex-col h-full bg-background/50">
				<div class="p-4 border-b border-border flex justify-between items-center">
					<div>
						<h3 class="font-bold">Visor de Sesión</h3>
		...
		<div class="flex flex-col h-full bg-background/50">
			...
			<div class="p-4 border-b border-border flex justify-between items-center">
				<div>
					...
					<span class="text-xs text-green-500">MODO EN VIVO ACTIVO</span>
				</div>
