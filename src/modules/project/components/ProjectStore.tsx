import { FormMKDEditor } from "@components/form/FormMKDEditor";
...
export default function ProjectStore({
	...
	const projectStoreForm = useForm<ProjectStoreDto>({
		resolver: zodResolver(projectStoreDto),
		mode: "all",
		defaultValues: {
			techeables: [],
		},
	...
	function onProjectStore(body: ProjectStoreDto) {
		const mappedBody = {
			...body,
			techeables:
				body.techeables?.map((techId) => ({
					technology_id: techId as unknown as number,
				})) || [],
		};

		projectStoreMutation.mutate(mappedBody as any, {
			onSuccess(response) {
				sweetModal({
					icon: "success",
				...
				refetch(response.project);
				onClose();
			},
			onError(error) {
				handlerError(projectStoreForm, error, "Sync Error");
			},
		});
	}
			...
			<Form {...projectStoreForm} onSubmit={onProjectStore}>
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
					<Form.control.area<ProjectStoreDto>
						name="impact_summary"
						title="RESULTS_IMPACT (SENIOR_RESULTS)"
