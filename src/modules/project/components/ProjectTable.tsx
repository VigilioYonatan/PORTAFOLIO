...
import ProjectUpdate from "@modules/project/components/ProjectUpdate";
import TechStackIcons from "@modules/project/components/TechStackIcons";
...
export default function ProjectTable() {
	...
	function handleRefetch(data: ProjectSchema) {
	...
	function handleStoreSuccess(data: any) {
		table.updateData((old, count) => ({
			result: [data, ...old],
			count: count + 1,
		}));
	...
	> = [
		...
		},
		{
			...
			isSort: true,
			cell: (row: ProjectSchema) => (
				<div class="flex items-center gap-1.5 font-mono text-[10px] font-bold text-primary/80">
					...
					<div class="flex items-center gap-1">
						<button
							...
							onClick={() => {
								...
								mutation.mutate({ is_visible: !row.is_visible } as any, {
									onSuccess() {
		...
		<VigilioTable table={table} query={query}>
			<div class="space-y-6">
					...
					<div class="bg-zinc-900/30 border border-white/5 p-4 rounded-2xl backdrop-blur-md relative overflow-hidden group">
						...
						<div class="flex items-center gap-3 relative">
							...
							<div>
								...
								<span class="text-xl font-black font-mono text-foreground">
									{query.data?.count ?? 0}
								</span>
							</div>
						</div>
				...
				<div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-zinc-900/40 border border-white/5 p-4 rounded-2xl backdrop-blur-xl shadow-2xl relative overflow-hidden">
					...
					<div class="relative group w-full lg:w-96 z-10">
						...
						<input
							...
							class="bg-black/60 border border-white/10 text-[10px] font-mono tracking-widest rounded-xl pl-24 pr-4 py-3 w-full focus:outline-none focus:ring-1 focus:ring-primary/40 focus:border-primary/30 transition-all placeholder:text-muted-foreground/20"
				...
				<Modal
					...
					onClose={() => (editingProject.value = null)}
