...
import type { WorkExperienceIndexMethods } from "@modules/work-experience/apis/work-experience.index.api";
...
import MilestoneList from "@modules/work-milestone/components/MilestoneList";
...
export default function ExperienceTimeline() {
	...
	const indexQuery = workExperienceIndexApi(table);
		...
		sweetModal({
			...
			if (isConfirmed) {
				destroyMutation.mutate(id, {
					onSuccess() {
						sweetModal({ icon: "success", title: "Eliminado con éxito" });
						indexQuery.refetch();
					},
					onError(err) {
						sweetModal({
	...
	if (indexQuery.isLoading) {
		return (
			<div class="flex flex-col items-center justify-center p-20 gap-4">
				<Loader />
				<span class="text-[10px] font-mono font-black tracking-[0.3em] text-primary/40">
		...
		return (
			<div class="p-10 border border-destructive/20 bg-destructive/5 text-destructive rounded-xl text-center font-mono">
				...
				<p class="text-[10px] mt-2 opacity-60">
					{(indexQuery.error as any)?.message ||
	...
	const experiences = (table as any).table.TBody.Row() as WorkExperienceSchema[];
	...
	return (
		<div class="space-y-8">
			...
			<div class="relative pl-8 md:pl-12 border-l-2 border-primary/20 space-y-12 ml-4">
				{experiences.length === 0 ? (
					<div class="p-10 text-center border border-dashed border-white/10 rounded-xl">
						<p class="text-[10px] font-mono font-bold text-muted-foreground/40 uppercase tracking-[0.3em]">
								...
								<div class="flex flex-col md:flex-row md:items-start justify-between gap-4 relative z-10">
									<div class="space-y-2">
										...
										<div class="flex flex-wrap items-center gap-4 mt-4">
											<div class="flex items-center gap-1.5 text-[10px] font-mono font-bold text-muted-foreground/60 uppercase">
									...
									<div class="flex items-center gap-2 self-start md:self-auto">
										<button
											type="button"
											onClick={() => (experienceEdit.value = exp)}
			...
			<Modal
				...
				onClose={() => (experienceEdit.value = null)}
				contentClassName="max-w-3xl w-full self-start"
			>
				<ExperienceUpdate
