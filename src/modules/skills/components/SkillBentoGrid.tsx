...
import { CodeIcon, ServerIcon, DatabaseIcon, ContainerIcon, SmartphoneIcon, BrainCircuitIcon } from "lucide-preact";

interface TechItem {
    name: string;
    icon: any; // Lucide icon or string url
            ...
            {CATEGORIES.map((category) => (
                ...
                >
                    ...
                    <div className="relative z-10 flex flex-col h-full">
                        ...
                        <p className="text-muted-foreground mb-6 text-sm">{category.description}</p>
                        
                        <div className="mt-auto grid grid-cols-2 md:grid-cols-3 gap-3">
                            {category.techs.map((tech) => (
                                <div key={tech.name} className="flex items-center gap-2 p-2 rounded bg-secondary/50 border border-white/5 hover:border-primary/30 transition-colors">
                                    <span className="text-lg">{tech.icon}</span>
                                    <div className="flex flex-col">
                                         <span className="font-semibold text-xs text-foreground">{tech.name}</span>
                                         <span className={cn("text-[10px]", getLevelColor(tech.level))}>{tech.level}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
