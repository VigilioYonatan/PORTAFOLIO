...
import { Circle, Trash2, Bell, Check } from "lucide-preact";
...
export function NotificationCenter() {
		...
		sweetModal({
			...
			if (isConfirmed) {
				destroyAllMutation.mutate(undefined, {
					onSuccess() {
						query.refetch(false);
						sweetModal({ icon: "success", title: "Cleared!" });
					},
				});
			}
		});
	...
	if (query.isLoading) return <div class="p-4 animate-pulse text-xs font-mono">LOADING_NOTIFICATIONS...</div>;
    if (query.isError) return <div class="p-4 text-xs font-mono text-red-500">ERROR_LOADING_STREAM</div>;
		...
		<div class="flex flex-col h-full bg-zinc-900/50 border border-white/5 rounded-xl overflow-hidden backdrop-blur-sm">
			<div class="flex items-center justify-between p-4 border-b border-white/5">
				<div class="flex items-center gap-2">
					...
					<h3 class="text-sm font-black font-mono tracking-widest uppercase">
			...
			<div class="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
				{notifications.length === 0 ? (
					<div class="h-full flex flex-col items-center justify-center text-zinc-600 gap-2">
						...
						<span class="text-xs font-mono">NO_NEW_SIGNALS</span>
					</div>
				) : (
					notifications.map((notif) => (
						<div
							...
							class={cn(
								...
								!notif.is_read ? "bg-primary/5 border-primary/10" : "opacity-60",
							)}
						>
							<div class="mt-1">
							...
							<div class="flex-1 min-w-0">
								<div class="flex items-start justify-between gap-2">
									<p class={cn("text-xs font-medium leading-none truncate", !notif.is_read ? "text-white" : "text-zinc-400")}>
										{notif.title}
									</p>
									<span class="text-[9px] text-zinc-600 font-mono whitespace-nowrap">
