...
import TechnologyStore from "@modules/technology/components/TechnologyStore";
import TechnologyUpdate from "@modules/technology/components/TechnologyUpdate";
...
export default function TechnologyTable() {
	...
	const columns: Columns<TechnologySchema, any, any> = [
		{
			key: "icon",
			header: "Visual",
			cell: (row) => (
				<div class="w-10 h-10 rounded-lg bg-zinc-900 border border-white/5 overflow-hidden flex items-center justify-center p-1.5">
					{row.icon ? (
						<img
							src={printFileWithDimension(row.icon, DIMENSION_IMAGE.sm)[0]}
							alt={row.name}
								...
								if (isConfirmed) {
									destroyMutation.mutate(row.id, {
										onSuccess() {
											...
											sweetModal({ icon: "success", title: "Record Deleted" });
										},
									});
								}
							});
	...
	const table = useTable<TechnologySchema, any, any>({
		columns,
		pagination: { limit: 12 },
	});
	...
	return (
		<VigilioTable table={table} query={query}>
			...
			<Modal
				...
				onClose={() => (editingTech.value = null)}
					...
					<TechnologyUpdate
						...
						refetch={(data) => {
							table.updateData((old, count) => ({
								result: old.map((item) =>
									item.id === (data as TechnologySchema).id ? { ...item, ...data } : item,
								),
								count,
							}));
