...
describe("ForgotPasswordForm", () => {
	...
	it("renders forgot password form correctly", () => {
		...
		expect(screen.getByText("auth.forgot.email_label")).toBeInTheDocument();
		expect(
			screen.getByRole("button", { name: /auth\.forgot\.submit/i }),
		).toBeInTheDocument();
	...
	it("shows loading state when submitting", () => {
		...
		expect(
			screen.getByRole("button", { name: /auth\.terminal\.processing/i }),
		).toBeInTheDocument();
	});
});
