...
export default function CategoryManager({ lang = "es" }: CategoryManagerProps) {
	...
	>({
		columns: [
			{ key: "id", header: t("dashboard.table.id"), isSort: true },
			{
				key: "name",
				...
				cell: (item) => (
					<div class="flex gap-2">
						...
						<button
							...
							onClick={() => {
								sweetModal({
									title: t("dashboard.blog.category.delete_title"),
									text: t("dashboard.blog.category.delete_text"),
									icon: "danger",
		...
		<div className="space-y-4">
			...
			<VigilioTable query={query} table={table}>
				<VigilioTable.table>
					...
					<VigilioTable.tbody>
						<VigilioTable.tbody.row title={t("dashboard.blog.category.empty")}>
