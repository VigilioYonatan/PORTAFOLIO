...
export default function AdminCard({ lang = "es" }: AdminCardProps) {
	...
	useEffect(() => {
		if (query.isSuccess && query.data?.user) {
			form.reset({
				username:
					(query.data.user as any).username || query.data.user.full_name,
	...
	function onSubmit(data: UserProfileUpdateDto) {
		updateMutation.mutate(data, {
			onSuccess() {
				sweetModal({
					icon: "success",
					title: t("dashboard.settings.profile_secured"),
					text: t("dashboard.settings.creds_synced"),
				});
				query.refetch();
			},
			onError(error) {
		...
		component = (
			<div class="bg-zinc-950/40 border border-white/5 rounded-3xl p-8 backdrop-blur-xl relative overflow-hidden group">
				...
				<div class="flex flex-col items-center mb-8 relative">
					...
					<h2 class="text-xl font-black tracking-tight text-foreground uppercase italic group-hover:text-primary transition-colors">
						{query.data?.user?.full_name}
					</h2>
					<span class="text-[9px] font-black tracking-[0.3em] text-muted-foreground/40 uppercase">
						{t("dashboard.settings.root_admin")}
					</span>
				</div>
					...
					<div class="space-y-6">
						<div class="space-y-4">
							<Form.control<UserProfileUpdateDto>
								name="username"
								title={t("dashboard.settings.system_handle")}
								ico={<Fingerprint size={16} />}
							/>
							<Form.control<UserProfileUpdateDto>
								name="email"
								title={t("dashboard.settings.sec_email")}
								ico={<Mail size={16} />}
							/>
							<Form.control<UserProfileUpdateDto>
								name="phone_number"
								title={t("dashboard.settings.voice_uplink_phone")}
								ico={<Phone size={16} />}
							/>
							<Form.control<UserProfileUpdateDto>
								name="password"
								title={t("dashboard.settings.access_upgrade")}
								type="password"
						...
						<div class="space-y-4">
							<Form.control.file<UserProfileUpdateDto>
								name="avatar"
								title={t("dashboard.settings.profile_image")}
								ico={<Camera size={16} />}
								entity="user"
								property="avatar"
								typeFile="image"
							...
							<div class="flex items-center gap-3 mb-6 p-4 bg-primary/5 rounded-2xl border border-primary/10">
								...
								<div>
									<span class="text-[9px] font-black uppercase tracking-widest text-primary block">
										{t("dashboard.settings.sec_clearance")}
									</span>
									<span class="text-[10px] text-muted-foreground">
										{t("dashboard.settings.admin_access")}
									</span>
								</div>
							</div>

							<Form.button.submit
								title={t("dashboard.settings.auth_changes")}
								loading_title={t("dashboard.settings.authorizing")}
