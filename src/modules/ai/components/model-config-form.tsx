...
export default function ModelConfigForm() {
	...
	if (query.isSuccess) {
		...
		const form = useForm<AiConfigUpdateDto>({
			resolver: zodResolver(aiConfigUpdateDto) as Resolver<AiConfigUpdateDto>,
			mode: "all",
			...
			<div class="space-y-6 border border-white/5 rounded-xl p-6 bg-card/30 backdrop-blur-md">
				...
				<Form {...form} onSubmit={onSubmit}>
					<div class="space-y-4">
						...
						<Form.control.toggle
							title="Núcleo Neuronal Activo"
							name="is_active"
							question="Habilitar/Deshabilitar funciones de IA globalmente"
						/>

						<Form.button.submit
							title="Actualizar Config Neuronal"
