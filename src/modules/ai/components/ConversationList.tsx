...
import { CalendarClock, MessageSquare, User } from "lucide-preact";
...
export default function ConversationList({
	...
	const table = useTable<any, any, any>({
		pagination: { limit: 20 },
		columns: [],
	});
	const query = conversationIndexApi(table);
			...
			<div class="flex-1 overflow-y-auto custom-scrollbar">
				{query.isLoading ? (
					...
					<div class="flex flex-col">
						{query.data?.results.map((chat: any) => (
							<div
								key={chat.id}
								...
								class={cn(
									...
									activeId === String(chat.id)
										...
										: "hover:bg-white/[0.02] border-l-2 border-l-transparent hover:border-l-primary/30",
								)}
							>
								{/* Active Indicator Background */}
								{activeId === String(chat.id) && (
									<div class="absolute inset-0 bg-gradient-to-r from-primary/10 to-transparent pointer-events-none" />
								)}

								<div class="relative z-10">
									<p
										class={cn(
											"text-xs font-bold mb-1 truncate transition-colors font-mono",
											activeId === String(chat.id)
												? "text-primary"
