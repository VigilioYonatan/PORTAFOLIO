...
function mapThemesToData(themes: string[]) {
	if (!themes || themes.length === 0)
		return [
			...
			{ subject: "Systems", A: 85, fullMark: 150 },
		...
		generateMutation.mutate(undefined, {
			onSuccess() {
				sweetModal({
					...
					text: "New recruiting insights generated successfully.",
				});
				query.refetch(false);
			},
			onError(err) {
		...
		<div class="mt-4 border border-white/5 rounded-xl p-4 bg-card/30 backdrop-blur-md">
			<div class="flex items-center justify-between mb-4">
				...
				>
					...
					{generateMutation.isLoading ? "COMPUTING..." : "RUN_ANALYSIS"}
				</Button>
			</div>
			...
			<div class="mt-4 grid grid-cols-2 gap-2">
				...
				<div class="p-3 bg-black/20 rounded border border-white/5">
					...
					<span class="text-xs font-mono text-zinc-300 line-clamp-2">
						{latestInsight?.insights_data.actions?.[0] || "AWAITING_DATA"}
					</span>
				</div>
			</div>
		</div>
