...
async function verify() {
  ...
  try {
    ...
    for (const row of results) {
      if (row.length < 5000) {
        console.warn(`❌ FAIL: [${row.language}] ${row.slug} - Length: ${row.length}`);
        failCount++;
      } else {
        console.log(`✅ PASS: [${row.language}] ${row.slug} - Length: ${row.length}`);
      }

      // Check for markdown titles (lines starting with #)
      if (row.content.split('\n').some((line: string) => line.startsWith('# '))) {
         console.warn(`⚠️ WARNING: [${row.language}] ${row.slug} contains internal # titles`);
         titleCount++;
      }
    }
