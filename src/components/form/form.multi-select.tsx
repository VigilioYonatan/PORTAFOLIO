	...
	return (
		<div class="space-y-2 w-full" ref={selectRef} {...rest}>
			...
			<div class="relative">
				...
				<div
					class={cn(
						"w-full py-2 border border-input rounded-(--radius-lg) text-foreground",
				...
				{isOpen.value && !disabled && (
					<div class="absolute z-10 w-full mt-1 border border-input rounded-(--radius-lg) shadow-lg max-h-60 overflow-auto bg-popover animate-in fade-in zoom-in-95 duration-100">
						{/* Options list */}
						{array.length > 0 ? (
							<ul>
								{array.map((option) => {
				...
				{question && (
					<div class="absolute right-10 top-1/2 -translate-y-1/2 group">
						...
						<div class="text-xs min-w-[100px] hidden group-hover:block -top-[35px] right-1 p-1 shadow text-center absolute rounded-(--radius-lg) text-popover-foreground z-10 font-semibold bg-popover border border-input">
							{question}
						</div>
					</div>
				)}
