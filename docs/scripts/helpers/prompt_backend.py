...
private readonly logger = new Logger(ExampleService.name);
 ...
 // esto es un index con paginador, claro si se usa pagina quiere decir que esta entidad tendrá muchos registros
  ...
  ): Promise<PaginatorResult<ExampleIndexSchema>> {
...
// index sin paginador, eso quiere decir que esta entidad no habrá un crud para agregar, pero si tiene un store crud para agregar si  o si usar paginacion
  async index(
		tenant_id: number,
	): Promise<{success:true, examples:ExampleIndexSchema[]}> {
	...
	async show(
		tenant_id: number,
		id: number,
	): Promise<{ success: true; example: ExampleShowSchema }> {
  ...
  async showBlogs(
    ...
    id: number,
    query: ExampleQueryDto,
  ): Promise<PaginatorResult<BlogSchema>> {
...
import { ExampleStoreClassDto } from "../dtos/example.store.class.dto";
...
export class ExampleController {
  ...
  ): Promise<{ success: true; data: ExampleIndexSchema[] }> {
...
// example.show.response.class.dto.ts
import { createZodDto } from "nestjs-zod";
...
export class UserService {
  ...
  async show(
    id: number,
    tenant_id: number,
  ): Promise<{ success: boolean; user: UserShowSchema }> {
  ...
  async store(
    tenant_id: number,
    body: UserStoreDto,
  ): Promise<{ success: boolean; user: UserSchema }> {
